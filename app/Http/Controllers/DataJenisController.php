...
class DataJenisController extends Controller
{
    public function index(){
      ...
      if (Session::get('menu_siswa')) {
        ...
        return view('SiswaModul.data-jenis')->with('menu_siswa' , $menu_siswa)->with('form_siswa' , $form_siswa);
      }else {
        $menu_admin = Session::get('menu_admin');
        $form_admin = Session::get('form_admin');
        return view('SiswaModul.data-jenis')->with('menu_admin' , $menu_admin)->with('form_admin' , $form_admin);
      }
